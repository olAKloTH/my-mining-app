...
export default function LoginPage() {
  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center justify-center bg-slate-950 p-4 overflow-hidden">
      {/* 1. Background & Gold Sparkles (ทองระยิบระยับ) */}
      ...
      >
        ...
        <div className="relative w-full max-w-md bg-slate-900/95 backdrop-blur-3xl p-8 rounded-[2.45rem] z-10 border border-white/5">
          ...
          <div className="text-center mb-8">
            {/* แก้ไขขนาดโลโก้ที่นี่: ปรับ container เป็น w-40 h-40 และปรับ Image width/height เป็น 240 */}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input type="email" required className="w-full mt-1.5 p-4 rounded-2xl bg-slate-950/50 border border-white/10 text-white focus:border-yellow-500/50 outline-none transition-all" placeholder="miner@goldglow.com" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 ml-2 uppercase tracking-widest">Password</label>
              <input type="password" required className="w-full mt-1.5 p-4 rounded-2xl bg-slate-950/50 border border-white/10 text-white focus:border-yellow-500/50 outline-none transition-all" placeholder="••••••••" />
            </div>
      ...
      {/* แทรกส่วนนี้เข้าไปเพื่อให้ animation spin ทำงาน */}
      <style jsx global>{`
        @keyframes spin {
