...
import { supabase } from '@/lib/supabaseClient'; // นำเข้า Supabase

export default function LoginPage() {
  const [isLoading, setIsLoading] = useState(false);
  const [mounted, setMounted] = useState(false);
  
  // เพิ่ม State สำหรับเก็บข้อมูลที่ผู้ใช้กรอก
  ...
  // ฟังก์ชัน Login ที่เชื่อม Supabase
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setErrorMsg("อีเมลหรือรหัสผ่านไม่ถูกต้อง"); // แจ้งเตือนถ้าล็อกอินพลาด
    } else {
      window.location.href = '/'; // ล็อกอินสำเร็จ ไปหน้าหลัก
    }
    setIsLoading(false);
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                placeholder="miner@goldglow.com" 
              />
            </div>
            <div>
